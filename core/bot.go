...
type Bot struct {
	...
	cmdHandlers       []func(string, func(string)) bool
...
func (bot *Bot) AddCmdHandler(cmdProcessCallback func(string, func(string)) bool, replyCallback func(string)) {
...
func (bot *Bot) mainHandler(event *irc.Event) {
	for i, handler := range bot.cmdHandlers {
		if handler(event.Message(), bot.cmdReplyCallbacks[i]) {
			return
		}
	}
