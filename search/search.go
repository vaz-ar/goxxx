...
func handleDuckduckGoCmd(event *irc.Event, callback func(*core.ReplyCallbackData)) bool {
	...
	if results == nil {
		callback(&core.ReplyCallbackData{Message: fmt.Sprintf("DuckDuckGo: No result for %q", message)})
		return true
	}
...
func handleUrbanDictionnaryCmd(event *irc.Event, callback func(*core.ReplyCallbackData)) bool {
	...
	if results == nil {
		callback(&core.ReplyCallbackData{Message: fmt.Sprintf("Urban Dictionnary: No result for %q", message)})
		return true
	}
	for index, item := range results {
		if index == 0 {
...
func handleWikipediaCmd(event *irc.Event, callback func(*core.ReplyCallbackData)) bool {
	...
	if results == nil {
		callback(&core.ReplyCallbackData{Message: fmt.Sprintf("Wikipedia: No result for %q", message)})
		return true
	}
	for index, item := range results {
		if index == 0 {
